...
## FASE 6 — Panel de administración propio en Vue (staff del restaurante)

Esta fase es opcional y se suma a Django Admin: crea una segunda aplicación Vue (o un área protegida dentro de la misma SPA) pensada para que el personal del restaurante gestione pedidos en tiempo real y el catálogo sin entrar al admin de Django. Requiere que en el backend (Fase 2) los usuarios de tipo staff puedan autenticarse igual que los clientes.

### 6.1 Backend — permisos y endpoints para staff

```
Fase 6.1 — Endpoints y permisos para el panel de staff.

Sobre el backend Django ya construido en las fases anteriores, agrega lo necesario para soportar un panel de administración propio consumido desde Vue:

1. En accounts/models.py, agrega un campo booleano "is_staff_restaurante" a CustomUser (independiente del is_staff de Django, para no mezclar con el acceso al admin nativo) o usa un Group de Django llamado "Staff" — elige la opción más simple y justifícala en una línea.

2. Crea una clase de permiso personalizada IsRestaurantStaff en core/permissions.py que valide que el usuario autenticado pertenezca a ese grupo/flag.

3. En orders/views.py agrega (protegidos con IsRestaurantStaff):
   - AdminOrderListAPIView: lista TODOS los pedidos (no solo los del usuario), con filtros por estado (?estado=), por rango de fechas (?desde=&hasta=) y ordenados por created_at descendente. Debe soportar paginación.
   - AdminOrderUpdateStatusAPIView: endpoint PATCH para cambiar el estado de un pedido puntual (valida que la transición de estado sea lógica, ej. no se puede pasar de "pendiente" a "entregado" sin pasar por los estados intermedios; deja la lista de transiciones válidas como una constante fácil de editar).
   - AdminDashboardStatsAPIView: devuelve métricas agregadas del día (cantidad de pedidos por estado, total vendido hoy, ticket promedio, producto más vendido de la semana) usando agregaciones de Django ORM (Count, Sum, Avg).

4. En catalog/views.py, agrega ViewSets con permisos de escritura para staff (no solo lectura como en la Fase 2): AdminProductViewSet, AdminCategoryViewSet, AdminComboViewSet, con operaciones CRUD completas (crear, editar, activar/desactivar, subir imagen).

5. Actualiza urls.py agrupando estos endpoints bajo el prefijo /api/admin-panel/ para diferenciarlos claramente de la API pública de clientes.

Dame el código completo de los archivos nuevos o modificados.
```

### 6.2 Frontend — layout y autenticación del panel

```
Fase 6.2 — Layout y login del panel de staff.

Dentro del mismo proyecto Vue (chifa-frontend), crea un área separada para el panel de administración, bajo la ruta base /panel, con su propio layout (distinto del sitio público):

1. src/views/admin/AdminLoginView.vue: formulario de login exclusivo para staff (mismo endpoint de auth, pero valida además que el usuario tenga el flag/grupo de staff; si no lo tiene, muestra "No tienes permisos para acceder al panel").

2. src/components/admin/AdminLayout.vue: layout con sidebar fijo (ícono + texto) con enlaces a: Dashboard, Pedidos, Productos, Categorías, Combos, Cupones, Zonas de delivery, y botón de cerrar sesión. Header superior con el nombre del usuario logueado y un indicador de "conectado" (punto verde).

3. Actualiza src/router/index.js agregando las rutas hijas de /panel (dashboard, pedidos, productos, categorías, combos, cupones, zonas), todas envueltas en AdminLayout.vue, protegidas con un meta { requiresStaff: true } y un navigation guard adicional que redirige a /panel/login si el usuario no es staff.

4. Crea src/stores/adminOrders.js (Pinia) para manejar el estado de pedidos del panel: lista de pedidos, filtros activos, acción fetchPedidos(filtros), acción actualizarEstado(pedidoId, nuevoEstado).

Dame el código completo de los archivos anteriores.
```

### 6.3 Frontend — dashboard y gestión de pedidos en tiempo casi real

```
Fase 6.3 — Dashboard y vista de pedidos del panel.

1. src/views/admin/DashboardView.vue: tarjetas de métricas (pedidos pendientes, pedidos del día, total vendido hoy, ticket promedio) consumiendo AdminDashboardStatsAPIView, y una tabla con los últimos 10 pedidos.

2. src/views/admin/OrdersManagementView.vue:
   - Tabs o filtro por estado (Pendientes, Confirmados, En preparación, En camino, Entregados, Cancelados).
   - Tabla/lista de pedidos con: número, cliente, tipo de entrega, total, hora, y un selector para cambiar el estado directamente desde la fila (usando adminOrders.actualizarEstado).
   - Al hacer clic en un pedido, abre un panel lateral (drawer) o modal con el detalle completo: items, extras, dirección, método de pago y comentarios del cliente.
   - Implementa actualización automática de la lista cada 30 segundos usando setInterval dentro de onMounted/onUnmounted (simulando "tiempo real" sin necesidad de WebSockets todavía), y agrega un comentario explicando cómo se podría migrar esto a Django Channels + WebSockets más adelante para notificaciones instantáneas de pedidos nuevos.

Dame el código completo de ambos archivos, con estilos Tailwind consistentes con el resto del panel.
```

### 6.4 Frontend — CRUD de catálogo (productos, categorías, combos, cupones, zonas)

```
Fase 6.4 — Gestión de catálogo desde el panel.

Crea las vistas de administración de catálogo, todas con un patrón consistente: tabla/listado con buscador y botón "Nuevo", formulario en modal o vista aparte para crear/editar, y confirmación antes de eliminar o desactivar:

1. src/views/admin/ProductsManagementView.vue + src/components/admin/ProductFormModal.vue: listado de productos con imagen miniatura, nombre, categoría, precio, estado (disponible/no disponible con un toggle rápido). El formulario permite subir imagen (input file), asignar categoría (select), definir precio base, variantes (repetidor dinámico de nombre + precio) y extras asociados (multi-select).

2. src/views/admin/CategoriesManagementView.vue: listado simple con reordenamiento (usar un campo "orden" con botones subir/bajar, sin necesidad de drag and drop para mantenerlo simple) y activar/desactivar.

3. src/views/admin/CombosManagementView.vue: similar a productos, pero el formulario permite seleccionar múltiples productos existentes y la cantidad de cada uno dentro del combo.

4. src/views/admin/CouponsManagementView.vue: listado de cupones con código, tipo de descuento, valor, vigencia y veces usado; formulario de creación/edición.

5. src/views/admin/DeliveryZonesManagementView.vue: listado y formulario simple de zonas de delivery (nombre, costo, tiempo estimado, activo).

Todas las vistas deben manejar estados de carga, mensajes de éxito/error tras guardar (puedes usar una notificación tipo "toast" simple reutilizable en src/components/admin/AdminToast.vue), y usar los endpoints Admin* creados en la Fase 6.1.

Dame el código completo de todos los archivos mencionados.
```
...
- La Fase 6 (panel propio en Vue) es un complemento a Django Admin, no un reemplazo: úsala si el dueño del restaurante necesita una interfaz más ágil para gestionar pedidos del día a día, mientras que Django Admin sigue siendo útil para tareas administrativas menos frecuentes.
- Una futura "Fase 7" natural sería migrar la actualización de pedidos del panel (hoy con polling cada 30s) a WebSockets reales con Django Channels, para notificaciones instantáneas de pedidos nuevos; el prompt 6.3 ya deja el punto de partida comentado en el código.
